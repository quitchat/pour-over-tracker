ALTER TABLE "CoffeeBean" ADD COLUMN "isDecaf" BOOLEAN NOT NULL DEFAULT false;
...
For country, return the bean's producing country or countries when clearly supported, such as Ethiopia, Colombia, Kenya, or Costa Rica. For multi-country coffees or blends with clear producing countries, separate countries with commas, for example Ethiopia, Colombia. Return null only when the producing country is unclear.

For origin, return the more specific origin or region/farm/cooperative text when clearly supported, without duplicating the country name or countries. For example, if the source says Ethiopia Yirgacheffe, return country as Ethiopia and origin as Yirgacheffe. If only the country is known, return country and set origin to null.
...
Return text facts only: country, origin/region, process, roast level, decaf status, flavor notes, source URL, and confirmed notes.$bean_prompt$,
