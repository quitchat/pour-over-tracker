...
- Two-step new-brew workflow: Brew Setup first, then Continue opens Brew Variables for grind size, water temperature, pour structure, total yield, and total brew time before the brew record is saved; the entry page auto-scrolls to the Brew Variables section and can load variables from the best prior brew with the same bean, grinder, and brewer, including showing that prior brew’s comments as reference while setting up the new brew
...
- Pour structure entry supports both manual notes and a guided pour plan with pour weight, target time, method rows, per-row add/remove controls, and editable parsing of saved guided rows
...
- User-level roaster consolidation captures selected old names before updating beans, prevents reverse alias cycles such as A → B and B → A, updates existing alias links when a preferred roaster is moved, and includes a cleanup migration that clears prior alias links so they can be rebuilt correctly.
